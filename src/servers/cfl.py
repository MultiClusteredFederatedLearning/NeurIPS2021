...
class ByzantineClusteredFederatedLearning:
    ...
    def optimal_bipartition(self, cluster, simmat):
        # & : np.intersect1d
        # | : np.union1d
        # - : np.setdiff1d
    ...
    def proposal_partition(self, cluster, simmat):
        # & : np.intersect1d
        # | : np.union1d
        # - : np.setdiff1d
    ...
    def clustering(self, thresh=0.02):
        M = len(self.clients)
        if M < 2:
            return
        ...
        # partition
        # C = self.optimal_bipartition(benign, simmat)
        C = self.proposal_partition(self.benign, simmat)
