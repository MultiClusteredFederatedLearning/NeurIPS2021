...
from servers.mcfl import ByzantineMultiClusteredFederatedLearning as MCFL
...
def main(cfg_path):
    ...
    history = np.zeros((cfg.n_rounds, 5)) # loss, accuracy, n_bengin, n_adv
    logger.debug("start training")
    logger.debug(f"| {'step':^10} | {'loss':^12} | {'accuracy':^12} | {'lr':^12} | {'n_cls':^6} | {'bengin':^6} | {'adv':^6}")

    # analysis
    simmats = np.zeros((cfg.n_rounds, len(clients), len(clients)))
    regmats = np.zeros((cfg.n_rounds, len(clients), len(clients)))
    ...
    for t in range(cfg.n_rounds):
        ...
        simmat, regmat, n_cls = server.clustering(thresh=cfg.thresh,
                                                    alpha=alpha_scheduler.alpha,
                                                    base_n_cls=cfg.base_n_cls,
                                                    reset=cfg.reset,
                                                    sqrt=cfg.sqrt)
        
        if cfg.adv == 'clean':
            server.benign = np.setdiff1d(server.benign, np.arange(n_noise))
            server.adv = np.union1d(server.adv, np.arange(n_noise))
        ...
        logger.debug(f"| {t:>10} | {round(loss, 7):>12} | {round(accuracy, 7):>12} | {round(lr, 7):>12} | {n_cls:>6} | {n_benign:>6} | {n_adv:>6}")

    np.save(os.path.join(logdir, 'history.npy'), history)
    np.save(os.path.join(logdir, 'simmat.npy'), simmats)
    np.save(os.path.join(logdir, 'regmat.npy'), regmats)


if __name__ == "__main__":
    try:
        mp.set_start_method('spawn')
        cfg_path = "../config/mcfl_cifar.yaml"
        main(cfg_path)
