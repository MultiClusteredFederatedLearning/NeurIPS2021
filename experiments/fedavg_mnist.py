...
from servers.fedavg import FedAvg
...
def main(cfg_path):
    ...
    history = np.zeros((cfg.n_rounds, 5)) # loss, accuracy
    logger.debug("start training")
    logger.debug(f"| {'step':^10} | {'loss':^12} | {'accuracy':^12} | {'lr':^12} ")
    ...
    for t in range(cfg.n_rounds):
        ...
        logger.debug(f"| {t:>10} | {round(loss, 7):>12} | {round(accuracy, 7):>12} | {round(lr, 7):>12} ")
...
if __name__ == "__main__":
    try:
        ...
        cfg_path = "../config/fedavg_mnist.yaml"
        main(cfg_path)
